...
function renderGuideFilters(){
  const tabs = [{k:'all', label:'הכל'}, ...GUIDES.map(g => ({k:String(g.id), label:g.subtitle})), {k:'allergies', label:'התאמות לאלרגיות', link:'allergies.html'}];
  guideFiltersEl.innerHTML = tabs.map(t =>
    `<button class="tab-btn ${activeGuide===t.k?'active':''}" data-guide="${t.k}" ${t.link?`data-link="${t.link}"`:''}>${t.label}</button>`
  ).join('');
